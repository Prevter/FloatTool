        ...
        private void StartSearchButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (ViewModel.CanEditSettings)
            {
                ...
                new Thread(() =>
                {
                    ...
                    // === FIX FOR STEAM API === //
                    // 1. Count can now only be 10, 20 or 100 (everything else will error)
                    if (ViewModel.SkinCount < 10)
                        ViewModel.SkinCount = 10;
                    else if (ViewModel.SkinCount < 20)
                        ViewModel.SkinCount = 20;
                    else
                        ViewModel.SkinCount = 100;

                    // 2. Start field is required to be multiple of count
                    if (ViewModel.SkinSkipCount % ViewModel.SkinCount != 0)
                        ViewModel.SkinSkipCount = (ViewModel.SkinSkipCount / ViewModel.SkinCount) * ViewModel.SkinCount;
                    // ============================

                    string url = $"https://steamcommunity.com/market/listings/730/{ViewModel.FullSkinName}/render/?query=&start={ViewModel.SkinSkipCount}&count={ViewModel.SkinCount}&currency={(int)AppHelpers.Settings.Currency}";
                    try
                    {
                        ...
                        try
                        {
                            r = JsonConvert.DeserializeObject(responseBody);
                        }
                        catch (JsonReaderException ex)
                        {
                            Logger.Error($"Error parsing JSON response from Steam: {ex}\n{responseBody}");
                            throw new ValueUnavailableException("Steam server returned error.");
                        }
